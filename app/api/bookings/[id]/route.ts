...
export async function PATCH(
  ...
  try {
    ...
    const updated = await Booking.findByIdAndUpdate(
      ...
      .populate("unit", "title pricePerMonth images rentalType")
      .populate("room", "label pricePerNight images")
    ...
    // Update the room's denormalized occupancy pointer when a short_term
    // booking is confirmed or cancelled — keep this in sync here since
    // Room.currentTenant is a convenience field, not the source of truth.
    if (updated.rentalType === "short_term" && updated.room) {
      const Room = (await import("@/models/Room")).default;
      if (status === "confirmed") {
        await Room.findByIdAndUpdate(updated.room._id, { currentTenant: updated.tenant._id });
      } else if (status === "cancelled") {
        await Room.findByIdAndUpdate(updated.room._id, { currentTenant: null });
      }
    }

    const listingName = updated.unit?.title ?? updated.room?.label ?? "the listing";
    const guestName    = updated.tenant?.name ?? "A guest";
    const moveInFmt    = new Date(updated.moveInDate).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" });
    const moveOutFmt   = new Date(updated.moveOutDate).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" });
