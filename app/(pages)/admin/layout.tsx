...
// Was hardcoded to a stale 4-position subset ("property_manager",
// "receptionist", "caretaker", "accountant") that had drifted out of sync
// with lib/roles.ts's actual STAFF_POSITIONS (6 positions, also includes
// "security" and "maintenance"). Now derived from the shared source so
...
const NAV_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      {
        // The building's individual 2-bedroom units — long/mid/short term.
        label: "Units",
        href: "/admin/units",
        icon: <Icon.Units />,
      ...
      {
        // Bookable bedrooms within short-term units.
        label: "Rooms",
        href: "/admin/rooms",
        icon: <Icon.Rooms />,
        allowedRoles: ["admin", "property_manager", "caretaker"] as StaffRole[],
...
const ROLE_META: Record<StaffRole, { label: string; color: string }> = {
  admin:             { label: "Administrator",    color: "bg-[#7A1B0F]"    },
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Same pattern Navbar.tsx already uses for the tenant-facing avatar.
  const userPhoto = (session?.user as { photo?: string })?.photo ?? "";
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-5 border-b border-slate-800">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 bg-[#7A1B0F] rounded-lg flex items-center justify-center text-white font-bold text-sm">Jluv</div>
          <div>
            <h1 className="text-white font-bold text-base leading-none">
              Jluv<span className="text-[#7A1B0F]/70">Stays</span>
            </h1>
                    ...
                    >
                      ...
                      {badge != null && badge > 0 && (
                        <span className={`
                          ...
                          ${isActive ? "bg-white/20 text-white" : "bg-[#7A1B0F] text-white"}
                        `}>
                          {badge > 99 ? "99+" : badge}
                        </span>
                      )}
      ...
      {/* User footer — now links to the profile page, and shows the
          user's actual photo when available instead of always falling
