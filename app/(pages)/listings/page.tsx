...
// Normalized shape the grid actually renders.
interface Listing {
  _id: string;
  name: string;
  description: string;
...
function roomToListing(r: RoomApiShape): Listing {
  return {
    ...
    // Date-range availability is handled by the booking system.
    isAvailable:
...
//
// IMPORTANT:
// This page wraps ListingsContent in Suspense because ListingsContent uses
// useSearchParams(). Next.js 16 requires useSearchParams() to be underneath
...
function ListingsContent() {
  ...
  const fetchListings = useCallback(
    async (
      ...
      if (moveInVal) {
        dateParams.set("moveIn", moveInVal);
      }

      if (moveOutVal) {
        dateParams.set("moveOut", moveOutVal);
      }

      if (min) {
        dateParams.set("minPrice", min);
      }

      if (max) {
        dateParams.set("maxPrice", max);
      }
      ...
      try {
        ...
        if (
          ...
          unitParams.set("rentalType", type);
        }

        const [unitsRes, roomsRes] =
          await Promise.all([
            wantUnits
              ? fetch(
                  `/api/units?${unitParams.toString()}`
                ).catch(() => null)
        ...
        if (!unitsData.success && wantUnits) {
          setError(
            (prev) =>
              prev ||
              "Long/Mid Term listings aren't available right now."
          );
        }
      } catch (err) {
        console.error(
  ...
  useEffect(() => {
    ...
    // Only run on mount.
    // Subsequent fetches are triggered explicitly by Search/Clear.
  ...
  const handleSearch = () => {
    // Only enforce duration when a specific rental type is selected
    // and both dates have been supplied.
    if (
      rentalType !== "all" &&
      moveIn &&
      moveOut
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
          ...
          {rentalType !== "all" && (
            <p className="text-xs text-gray-500 mb-4">
              ℹ️{" "}
              {DURATION_RULES[rentalType].helperText}
            </p>
          )}
